(ns mineralwoolmfg.store
  "SSoT for the ISIC 2399 (Manufacture of other non-metallic mineral
  products n.e.c.) plant-operations coordination actor, illustrated
  concretely with a mineral wool / rock wool insulation plant, behind
  ...
  modeled on `cloud-itonami-isic-2392`'s `claymfg.store`), this build
  ...
  self-contained (no external mineral-wool-insulation capability
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch (a
                             forming/curing batch of mineral wool
                             insulation)'s product-type/weight/
                             thermal-conductivity/density record.
    ...
    - `equipment`         -- a forming/curing-line unit's own record
                             (cupola melter, fiberizing spinner, binder
                             applicator, curing oven, cutting line).
                             ...
                             `batches`.
    - `maintenance`       -- a scheduled forming/curing-line-equipment
                             maintenance-window DRAFT against a piece
                             of equipment (`mineralwoolmfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound mineral wool
                             insulation shipment DRAFT
                             (`mineralwoolmfg.registry`'s
                             `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (mineralwoolmfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (mineralwoolmfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-type :batt-insulation
                :material "R-13 Batt Insulation"
                :weight-kg 40000.0 :thermal-conductivity-w-mk 0.035 :density-kg-m3 16.0
                :verified? true :registered? true
                :shipped-weight-kg 8000.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :product-type :rigid-board-insulation
                :material "High-Density Rigid Board Insulation"
                :weight-kg 6000.0 :thermal-conductivity-w-mk 0.038 :density-kg-m3 120.0
                :verified? true :registered? true
                :shipped-weight-kg 5600.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :product-type :loose-fill-insulation
                :material "Loose-Fill Blown Mineral Wool"
                :weight-kg 50000.0 :thermal-conductivity-w-mk 0.040 :density-kg-m3 20.0
                :verified? false :registered? false
                :shipped-weight-kg 0.0
                :last-assessed "2026-05-15"}})

(defn- sample-equipment []
  {"forming-line-001" {:id "forming-line-001" :kind :fiberizing-spinner
                        :verified? true :registered? true
                        :last-maintenance-date "2026-05-01"}
   "curing-oven-002" {:id "curing-oven-002" :kind :curing-oven
                       :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `mineralwoolmfg.registry` and
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `mineralwoolmfg.registry` and
  returns {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  coordinated against it); one verified+registered fiberizing-spinner
  unit (schedulable for maintenance), one UNVERIFIED/unregistered
  curing-oven unit (blocks any maintenance scheduling against it) --
