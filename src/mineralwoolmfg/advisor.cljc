(ns mineralwoolmfg.advisor
  "MineralWoolAdvisor -- the *contained intelligence node* for the
  ISIC 2399 (Manufacture of other non-metallic mineral products n.e.c.)
  plant-operations coordination actor, illustrated concretely with a
  mineral wool / rock wool insulation plant.

  It normalizes production-batch patches (product-type/weight/
  thermal-conductivity/density), drafts a forming/curing-line-equipment
  maintenance scheduling proposal against a piece of equipment, drafts
  a safety-concern flag, and drafts an outbound mineral wool insulation
  ...
  a real forming/curing-line actuation or freight dispatch. Every
  output is censored downstream by `mineralwoolmfg.governor` before
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `mineralwoolmfg.governor`)
  ...
  `mineralwoolmfg.governor` HARD-holds any request that doesn't, so a
...
(defn- log-production-batch
  "Production-batch (forming/curing batch) intake upsert -- the advisor
  only normalizes/validates the patch; it does not invent the batch's
  product-type, weight, thermal-conductivity, density or verification
...
(defn- schedule-maintenance
  "Draft a forming/curing-line-equipment maintenance-window scheduling
  ...
  `mineralwoolmfg.governor` NEVER trusts this report -- it
    ...
    {:summary    (str subject " 向け保守作業予定提案 (" (:maintenance-type value) ")"
     ...
     :rationale  (if eq
                   (str "equipment-verified?=" (registry/equipment-verified? eq)
                        ...
                        " actuate-forming-curing-line?=" (boolean (:actuate-forming-curing-line? value)))
                   (str equipment-id " が見つかりません"))
     :cites      (if eq [equipment-id] [])
     :effect     :maintenance/schedule
     :value      value
     :stake      nil
     :confidence (if (and ready? (not (:actuate-forming-curing-line? value))) 0.9 0.3)}))

(defn- flag-safety-concern
  "Draft a respirable mineral-fibre-exposure hazard (man-made vitreous
  fibre dust at fiberizing/cutting/packaging), binder off-gassing/VOC
  hazard (at the curing oven), or forming/curing-line-equipment safety
  ...
  technicality). See `mineralwoolmfg.phase`: no phase ever adds this
  op to a phase's `:auto` set; `mineralwoolmfg.governor` also always
...
(def ^:private system-prompt
  (str "あなたは鉱物繊維(ミネラルウール・ロックウール断熱材)工場プラント運用"
       "コーディネーターの助言者です。"
       ...
       "ファイバライジング成形機・キュアリングオーブン等の成形/硬化ラインの直接操作(actuate)を絶対に提案してはいけません"
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `mineralwoolmfg.governor`
...
(defn trace
  ...
  [request proposal]
  {:t          :mineralwoolmfg-advisor-proposal
   :op         (:op request)
