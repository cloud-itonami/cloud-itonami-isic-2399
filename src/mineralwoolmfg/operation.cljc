(ns mineralwoolmfg.operation
  "MineralWoolOperationActor -- one plant-operations coordination
  request = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (MineralWoolAdvisor) is sealed into a single
  node (:advise); its proposal is ALWAYS routed through the Mineral
  Wool Plant Operations Governor (:govern) and the rollout phase gate
  ...
  (:require [langgraph.graph :as g]
            ...
            [mineralwoolmfg.advisor :as advisor]
...
(defn build
  "Compiles a MineralWoolOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `mineralwoolmfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; MineralWoolAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Mineral Wool Plant Operations Governor -- independent censor.
      (g/add-node :govern
