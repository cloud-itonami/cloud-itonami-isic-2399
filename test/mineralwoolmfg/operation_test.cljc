(ns mineralwoolmfg.operation-test
  "Smoke tests for the compiled MineralWoolOperationActor graph itself
  (build + one happy path per op). The governor's full rule contract
  (HARD holds, escalation, phase gating) is exercised in
  `mineralwoolmfg.governor-contract-test`; the Store contract in
  `mineralwoolmfg.store-contract-test`."
...
(deftest test-actor-builds
  (testing "MineralWoolOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t1"
                          {:op :log-production-batch :effect :propose :subject "batch-001"
                           :patch {:product-type :rigid-board-insulation}}
                          coordinator)
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "forming-line-001" :severity :moderate :description "respirable mineral-fibre dust rise near fiberizing spinner"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
